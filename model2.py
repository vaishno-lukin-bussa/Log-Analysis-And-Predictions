...
MODEL_PATH = os.path.join(MODEL_DIR, "log_lstm_model.h5")
SCALER_PATH = os.path.join(MODEL_DIR, "scaler.save")

log_file = st.file_uploader("Upload .log file", type=["log", "txt"])

if log_file:
    content = log_file.read().decode("utf-8").splitlines()
    logs = []
    for line in content:
        match = re.match(r"^([\d\-]+\s[\d:,]+)\s*-\s*(\w+)\s*\[(.*?)\]\s*-\s*(.*)", line)
        if match:
            timestamp, level, thread, message = match.groups()
            logs.append((timestamp, level.strip(), thread.strip(), message.strip()))
    
    if not logs:
        st.error("No valid logs found.")
        st.stop()

    df = pd.DataFrame(logs, columns=["Timestamp", "Level", "Thread", "Message"])
